...
const renderPartial = (props: any, onLoad: any)=>{
  return <WmLabel  name={'PartialLabel'}
  caption={`Partial label caption`}></WmLabel>
}

const CompWithPartial = {
  children: generateAccordionPane(1, {onLoad: jest.fn()}, renderPartial, true)
} as WmAccordionProps
...
const getTitles = ()=>{
  return Array.from(CompWithChildrens.children)
      .map((pane:any) => pane.props.title); 
}

const getSubHeading = ()=>{
  return Array.from(CompWithChildrens.children)
      .map((pane:any) => pane.props.subheading); 
}

const getBadgeValues = ()=>{
  return Array.from(CompWithChildrens.children)
      .map((pane:any) => pane.props.badgevalue); 
}
...
describe('WmAccordion Component', ()=>{
  ...
  test('check for title, subheading rendered properly', ()=>{
    ...
    const sub_headings =  panes_header_subheading.map(sub_heading => sub_heading.props.children)
  ...
  test('check for titleIcon rendered properly', ()=>{
    ...
    const accodionpane_icons = getAllByTestId(/^accordion\d_titleIcon\d+_icon$/)
  ...
  test('should handle current item expanded correctly', async() => {
    ...
    const { getByTestId, getByText} = renderComponent({...CompWithChildrens, ref})
    ...
    for(let i=0; i< getAccordionPanes().length; i++){
      await waitFor(()=>{
        ...
        if(i == currentItemIndex) expect(renderedHeaderStyles).toContainEqual(expect.objectContaining(expectedHeaderStyles))
        else expect(renderedHeaderStyles).not.toContainEqual(expect.objectContaining(expectedHeaderStyles))
        
        // check for accordionpane children displaying properly
        const contentLabel = getByTestId(`accordionpane${i+1}_content_label${i+1}`);
        const styleObj = i == currentItemIndex ? {} : { maxHeight:0, overflow:'hidden' }
      ...
      const renderedSubHeadingStyles = subHeading.props.style
        ...
        await waitFor(()=>{
          if( i == currentItemIndex) {
            expect(renderedTitleStyles).toContainEqual(expect.objectContaining(expectedTitleStyles))
            expect(renderedSubHeadingStyles).toContainEqual(expect.objectContaining(expectedSubHeadingStyles))
          } else {
            expect(renderedTitleStyles).not.toContainEqual(expect.objectContaining(expectedTitleStyles))
            expect(renderedSubHeadingStyles).not.toContainEqual(expect.objectContaining(expectedSubHeadingStyles))
          }
        })
    }
  })
  ...
  test('check for collapse method', async()=>{
    ...
    const contentLabel = screen.getByTestId(`accordionpane2_content_label2`);
    await waitFor(()=>{
      expect(contentLabel.props.style).toEqual({ maxHeight: 0, overflow: 'hidden' })
    })
  })

  test('check for onChange in accordion', async()=>{
    const onChangeMock = jest.fn()
    const tree = renderComponent({...CompWithChildrens,  onChange: onChangeMock })
  ...
  test('check for onExpand and onCollpase in accordionpane', async()=>{
    ...
    const CompWithOnExpandCollpaseFn = {
      children: generateAccordionPane(childrensLength, {onExpand: onExpandMock, onCollapse: onCollapseMock})
    }
    // Simulating as web preview and expanding accordionpane
    Platform.OS = 'web';
    const ref = createRef()

    const { getByText} = renderComponent({...CompWithOnExpandCollpaseFn, ref, onChange: onChangeMock})
    const currentItemIndex = childrensLength - 2 ;
    fireEvent.press(getByText(`Title${currentItemIndex + 1}`));
    await timer(1000)
  ...
  test('render skeleton if showskeleton is true and showskeletonchildren is true', async() => {
    const renderSkeletonMock = jest.spyOn(WmAccordion.prototype, 'renderSkeleton');

    const tree = renderComponent({...CompWithChildrens, showskeleton:true , showskeletonchildren:true})
  ...
  test('render skeleton if showskeleton is true and showskeletonchildren is false', async() => {
    ...
    const tree = renderComponent({...CompWithChildrens, showskeleton:true , showskeletonchildren:false})
    expect(screen).toMatchSnapshot();
