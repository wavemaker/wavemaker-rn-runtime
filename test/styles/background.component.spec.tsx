...
describe('Background Component', () => {
  ...
  test('should apply image natural width and height when size is falsy', async () => {
    ...
    await waitFor(() => {
      ...
      expect(tree.UNSAFE_getByType(Image).props.style).toEqual(expect.arrayContaining([{
        "minHeight": 200,
        "minWidth": 100,
      }]));
    });
  ...
  test('should apply position center correctly', async () => {
    const props = {
      image: 'url(http://example.com/image.jpg)',
      position: 'center',
    };
  ...
  test('should render when position has two values', async () => {
    const tree = renderComponent({
      image: 'http://example.com/image.jpg',
      position: "(top, left)"
    });
    ...
    await waitFor(() => {
      ...
      expect(tree.UNSAFE_getByType(Image).parent?.props.style).toEqual(expect.arrayContaining([{
        left: 0,
        top:0
      }]));
      expect(tree).toMatchSnapshot();
