...
import {
  StyleProps,
  getStyleName,
} from '@wavemaker/app-rn-runtime/styles/style-props';
...
> extends React.Component<T, S> {
  ...
  constructor(
    ...
    this.propertyProvider = new PropsProvider<T>(
      ...
      (name: string, $new: any, $old: any) => {
        WIDGET_LOGGER.debug(
          () =>
            `${this.props.name || this.constructor.name}: ${name} changed from ${$old} to ${$new}`
        );
        if (this.initialized) {
  ...
  updateState(newPartialState: S, callback?: () => void) {
    ...
    const stateFn = (oldState: S) => {
      ...
      const newState = this.initialized
        ? assignIn({}, oldState, newPartialState)
        : assignIn(oldState, newPartialState);
    ...
    if (!this.initialized) {
      ...
      const timeoutId = setTimeout(() => {
        ...
        this.updateStateTimeouts.splice(
          this.updateStateTimeouts.indexOf(timeoutId),
          1
        );
      });
  ...
  shouldComponentUpdate(nextProps: T, nextState: S, nextContext: any) {
    ...
    for (let key in nextState) {
      if (
        key !== 'props' &&
        (!(key in this.state) || nextState[key] !== this.state[key])
      ) {
        return true;
      }
    }
    ...
    for (let key in this.state) {
      if (
        key !== 'props' &&
        (!(key in nextState) || this.state[key] !== nextState[key])
      ) {
        return true;
      }
    }
  ...
  renderFixedContainer(props: T) {
    ...
    return (
      <FixedView style={style} theme={this.theme}>
  ...
  public render(): ReactNode {
    ...
    return this.getDependenciesFromContext(() => {
      ...
      this.styles = this.theme.mergeStyle(
        ...
        props.disabled
          ? this.theme.getStyle(this.defaultClass + '-disabled')
          : null,
        ...
        props.showindevice &&
          this.theme.getStyle(
            'd-all-none ' +
              props.showindevice.map((d) => `d-${d}-flex`).join(' ')
          ),
