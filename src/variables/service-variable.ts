...
export class ServiceVariable extends BaseVariable {
    ...
    invoke(options? : any) {
        ...
        if (includes(config.serviceInfo.methodType.consumes, WS_CONSTANTS.CONTENT_TYPES.FORM_URL_ENCODED)
          && !includes(WS_CONSTANTS.NON_BODY_HTTP_METHODS, (config.serviceInfo.methodType || '').toUpperCase())) {
        ...
        return axios[config.serviceInfo.methodType](url, requestBody, {headers: headers}).then(result => {
            ...
            config.onError && config.onError(this, null, error);
            return this;
        });
    }
