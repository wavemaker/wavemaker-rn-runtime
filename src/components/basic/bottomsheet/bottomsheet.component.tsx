...
import { Text, View, Animated, PanResponder, Dimensions, TouchableWithoutFeedback, Platform, ScrollView,SafeAreaView } from 'react-native';
...
export default class WmBottomsheet extends BaseComponent<WmBottomsheetProps, WmBottomsheetState, WmBottomsheetStyles> {
  ...
  private calculateSheetHeight(sheetheightratio: number): number {
    ...
    if (Platform.OS === 'ios') {
      const defaultTopInset = 44; // Common value for iPhone X and newer
      // Only subtract safe area inset if sheet height would reach into safe area
      // For example, if sheet is 90% of screen height, it will reach into safe area
      if (maxHeightRatio >= 0.9) {
        calculatedHeight -= defaultTopInset;
      }
    }
    
    console.log("device height is", SCREEN_HEIGHT);
    console.log("0.9  height is", SCREEN_HEIGHT*0.9);
...
// import React, { createRef } from 'react';
// import { BaseComponent, BaseComponentState } from '@wavemaker/app-rn-runtime/core/base.component';
// import { Text, View, Animated, PanResponder, Dimensions, TouchableWithoutFeedback, Platform, ScrollView } from 'react-native';
// import WmBottomsheetProps from './bottomsheet.props';
// import { DEFAULT_CLASS, WmBottomsheetStyles } from './bottomsheet.styles';

// const { height: SCREEN_HEIGHT } = Dimensions.get('screen');
// const EXPANDED_HEIGHT = SCREEN_HEIGHT * 0.8; // 80% of screen height
// const ANIMATION_DURATION = 400; // Animation duration in milliseconds
...
//     // Remove the artificial 100px minimum and allow full screen height
//     const calculatedHeight = SCREEN_HEIGHT * maxHeightRatio;
...
//         const newTranslateY = Math.max(0, this.state.lastGestureDy + gestureState.dy);
//         this.state.translateY.setValue(newTranslateY);
//       } else if (gestureState.dy < 0) { // Handle upward drag - expand to full height
//         // Allow expansion to full screen height
//         const targetHeight = Math.min(this.expandedHeight, SCREEN_HEIGHT);
//         Animated.timing(this.state.sheetHeight, {
//           toValue: targetHeight,
