...
import { Text, View, TouchableOpacity, Dimensions, TouchableWithoutFeedback, Image } from 'react-native';
...
export default class WmTabbar extends BaseNavComponent<WmTabbarProps, WmTabbarState<WmTabbarProps>, WmTabbarStyles> {
  ...
  private returnpathDown = getPathDown(this.maxWidth, 65 ,60);
  ...
  renderTabItem(item: TabDataItem, testId: string, props: WmTabbarProps, onSelect: Function, floating = false) {
    ...
    return (
      <View style={[increasedGap, floating? this.styles.centerHubItem: {}]} key={`${item.label}_${testId}`}>
        ...
        <Text style={[this.styles.tabLabel, floating? this.styles.centerHubLabel: {} , isActive && !floating  ? this.styles.activeTabLabel : {}]}>
  ...
  renderWidget(props: WmTabbarProps) {
    ...
    return (
      <NavigationServiceConsumer>
        ...
        {isClippedTabbar ? (
        <Svg width={this.maxWidth} height={scale(100)} style={{zIndex: -1,position: 'absolute'}}>
